...
本章定义了DMS文档中使用的技术术语和概念，帮助您理解相关内容。
...
认证，验证用户或系统的身份。DMS支持JWT和OAuth2等认证方式。
...
构建器模式，一种创建复杂对象的设计模式。DMS使用`DMSAppBuilder`实现构建器模式。
...
缓存，临时存储频繁访问的数据，减少对后端系统的访问。DMS支持多种缓存后端。
...
核心模块，DMS的基础模块，提供运行时、错误处理和服务上下文等功能。
...
DMS的统一错误类型，包含错误代码、消息和上下文信息。

### DMSResult

结果类型别名，定义为`Result<T, DMSError>`，简化错误处理。
...
文件系统，管理文件和目录的系统。DMS提供安全的文件系统操作。

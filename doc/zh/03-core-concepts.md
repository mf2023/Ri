...
- 支持同步和异步服务模块
- 版本：**0.1.6**（需 Python 3.8+）

### 1.6 C/C++ 模块支持

DMSC 通过 `safer-ffi` crate 提供 C/C++ FFI 绑定，支持跨语言集成：

| C/C++ 模块 | 对应 Rust 模块 | 描述 |
|:-----------|:--------------|:-----|
| `dmsc_c` | core | 核心运行时和服务上下文 |
| `dmsc_c::auth` | auth | 认证与授权 |
| `dmsc_c::cache` | cache | 多后端缓存抽象 |
| `dmsc_c::config` | config | 配置管理 |
| `dmsc_c::database` | database | 数据库操作 |
| `dmsc_c::device` | device | 设备管理 |
| `dmsc_c::fs` | fs | 文件系统操作 |
| `dmsc_c::gateway` | gateway | API 网关功能 |
| `dmsc_c::log` | log | 结构化日志 |
| `dmsc_c::observability` | observability | 指标和追踪 |
| `dmsc_c::queue` | queue | 消息队列操作 |
| `dmsc_c::service_mesh` | service_mesh | 服务网格功能 |

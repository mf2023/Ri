...
**Ri (Ri)** — 一个高性能的 Rust 中间件框架，统一后端基础设施。专为企业级规模构建，具有模块化架构、内置可观测性和分布式系统支持。
...
### 📐 模块化设计
Ri 采用高度模块化的架构，拥有 18 个核心模块及可选扩展模块，支持按需组合和无缝扩展：
...
**Q: 如何添加新模块？**
A: 实现 `RiModule` trait 并通过 `RiAppBuilder::with_module` 注册。
...
**Q: 如何扩展配置源？**
A: 实现自定义配置加载器并用 `RiConfigManager` 注册。

**Q: 如何处理异步任务？**
A: 使用 `RiAppBuilder::with_async_module` 添加异步模块，框架自动处理异步生命周期。

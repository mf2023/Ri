...
<h1 style="display: flex; flex-direction: column; align-items: center; gap: 12px; margin-bottom: 8px;">
  ...
  <span style="font-size: 0.6em; color: #666; font-weight: normal;">适用于 Python 的 Ri 库</span>
</h1>
...
**Ri (Ri)** — 一个高性能的 Rust 中间件框架，带有 Python 绑定。专为企业级规模构建，具有模块化架构、内置可观测性和分布式系统支持。
...
> **注意**：在 Python 中使用 gRPC/WebSocket 服务器，请使用原生 Python 库如 `grpcio` 和 `websockets`。Ri Rust API 为高性能场景提供高级功能。
...
Ri 提供了全面的认证支持，包括 JWT、会话、OAuth 和 API 密钥。
...
config = RiAuthConfig(
    enabled=True,
    jwt_secret="your-secret-key",  # 可选，默认从 Ri_JWT_SECRET 环境变量加载
...
- **TestRiAppBuilder**：应用程序构建器包装类的测试
- **TestRiAppRuntime**：应用程序运行时包装类的测试
- **TestRiAppBuilderWrapper**：Python 包装器行为测试（方法链式调用）
- **TestRiAppRuntimeWrapper**：运行时包装器行为测试

#### 包装类设计原理

`RiAppBuilder` 和 `RiAppRuntime` 类是 Rust 实现的 Python 包装器。这些包装器是必要的，因为：
...
**Q: 如何添加新模块？**
A: 使用现有的模块接口并通过 RiAppBuilder 注册。

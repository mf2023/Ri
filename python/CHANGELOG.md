...
- Added comprehensive Python import tests (tests/Python/ri_imports.py)
...
- Added `SECURITY.md`
- Added project logo `assets/svg/ri.svg`
...
- 新增 Python 导入测试 (tests/Python/ri_imports.py)
...
- Fixed module import path from `dms::prelude::*` to `ri::prelude::*`
...
#### 模块导入路径修复
- 修复模块导入路径从`dms::prelude::*`到`ri::prelude::*`
...
- Created modular Python submodule structure (ri.log, ri.config, ri.device, ri.cache, ri.fs, ri.hooks, ri.observability, ri.queue, ri.gateway, ri.service_mesh, ri.auth)
...
- Added precompiled Python wheel packages (ri_0.1.3_linux.whl, ri_0.1.3_windows.whl)
- Added precompiled Rust library files (libri.rlib, libri.dll)
...
- 创建模块化的Python子模块结构（ri.log、ri.config、ri.device、ri.cache、ri.fs、ri.hooks、ri.observability、ri.queue、ri.gateway、ri.service_mesh、ri.auth）
...
#### 预编译产物支持
- 新增预编译的Python wheel包（ri_0.1.3_linux.whl、ri_0.1.3_windows.whl）
- 新增预编译的Rust库文件（libri.rlib、libri.dll）
- 提供更便捷的部署和集成方式

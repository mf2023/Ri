 ...
 */
int main(void) {
    printf("=== RiAppBuilder C Binding Tests ===\n\n");
    ...
    RiAppBuilder* b1 = ri_app_builder_new();
    if (b1 != NULL) {
        RiAppBuilder* b2 = ri_app_builder_with_config(b1, "config1.yaml");
        RiAppBuilder* b3 = (b2 != NULL) ? ri_app_builder_with_config(b2, "config2.yaml") : NULL;
        ...
        ri_app_builder_free(b1);
        if (b2 != NULL) ri_app_builder_free(b2);
        if (b3 != NULL) ri_app_builder_free(b3);
    } else {
    ...
    RiAppBuilder* b = ri_app_builder_new();
    if (b != NULL) {
        RiAppRuntime* runtime = ri_app_builder_build(b);
        
        if (runtime != NULL) {
            printf("[PASS] testBuildCreatesRuntime: build() returns RiAppRuntime\n\n");
            passed++;
    ...
    RiAppBuilder* b_clean = ri_app_builder_new();
    if (b_clean != NULL) {
        ri_app_builder_free(b_clean);

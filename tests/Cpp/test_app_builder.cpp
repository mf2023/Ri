 ...
 */
int main() {
    std::cout << "=== DMSCAppBuilder C++ Binding Tests ===" << std::endl << std::endl;
    ...
    try {
        dmsc::DMSCAppBuilder builder1;
        dmsc::DMSCAppBuilder builder2 = builder1.with_config("config1.yaml");
        dmsc::DMSCAppBuilder builder3 = builder2.with_config("config2.yaml");
    ...
    try {
        ...
        std::cout << "[PASS] testBuildCreatesRuntime: build() returns DMSCAppRuntime" << std::endl << std::endl;
        passed++;
    } catch (const dmsc::DMSCError& e) {

...
int main(void) {
    ...
    DMSCAuthConfig* config = dmsc_auth_config_new();
    if (config == NULL) {
        printf("   [FAIL] Failed to create auth config\n");
        return 1;
    }

    dmsc_auth_config_set_jwt_secret(config, "your-secret-key-here");
    dmsc_auth_config_set_jwt_expiry_secs(config, 3600);
    dmsc_auth_config_set_session_timeout_secs(config, 86400);
    printf("   Auth config created\n\n");

    /* Initialize auth module */
    printf("2. Creating authentication module...\n");
    DMSCAuthModule* auth_module = dmsc_auth_module_new(config);
    if (auth_module == NULL) {
    ...
    char* token = dmsc_auth_module_generate_token(
        auth_module,
        user_id,
        roles,
        2,
    ...
    if (token != NULL) {
        bool is_valid = dmsc_auth_module_validate_token(auth_module, token);
        printf("   Token is valid: %s\n\n", is_valid ? "true" : "false");
        passed++;
        dmsc_string_free(token);
    } else {
    ...
    printf("   API keys enabled: %s\n", dmsc_auth_module_is_api_keys_enabled(auth_module) ? "true" : "false");
    printf("   Session auth enabled: %s\n\n", dmsc_auth_module_is_session_auth_enabled(auth_module) ? "true" : "false");

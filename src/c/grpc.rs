...
use crate::grpc::{DMSCGrpcChannel, DMSCGrpcClient, DMSCGrpcServer};


/// Opaque C wrapper structure for DMSCGrpcServer.
///
/// gRPC server implementation handling service registration, request processing, and response
/// streaming. The server manages the complete lifecycle of gRPC services.
///
/// # Server Responsibilities
///
/// The gRPC server manages:
///
/// - **Service Registration**: Register service implementations and method handlers
/// - **Connection Handling**: Accept and manage HTTP/2 connections from clients
/// - **Stream Multiplexing**: Handle multiple concurrent requests per connection
/// - **Method Dispatch**: Route requests to appropriate service handlers
/// - **Interceptor Execution**: Apply server interceptors to requests/responses
/// - **Graceful Shutdown**: Drain connections and release resources cleanly
///
/// # Server Lifecycle
///
/// The server follows this operational lifecycle:
///
/// 1. **Creation**: Initialize server with configuration (port, thread pool, TLS)
/// 2. **Registration**: Register services and their method handlers
/// 3. **Startup**: Begin listening for connections and processing requests
/// 4. **Operation**: Handle concurrent requests during normal operation
/// 5. **Shutdown**: Gracefully drain requests and close connections
/// 6. **Cleanup**: Release all server resources
///
/// # Thread Safety
///
/// The gRPC server is fully thread-safe:
///
/// - Service registration during startup is thread-safe
/// - Request handlers are invoked concurrently
/// - Interceptors execute in the context of each request
/// - Metrics collection is thread-safe
c_wrapper!(CDMSCGrpcServer, DMSCGrpcServer);

/// Opaque C wrapper structure for DMSCGrpcClient.
///
/// gRPC client implementation for invoking remote procedures with automatic serialization,
/// connection pooling, and retry logic.
///
/// # Client Responsibilities
///
/// The gRPC client manages:
///
/// - **Channel Management**: Maintains connection to server through channel
/// - **Request Serialization**: Encodes requests using Protocol Buffers
/// - **Method Invocation**: Dispatches requests using appropriate calling pattern
/// - **Response Handling**: Decodes responses and handles errors
/// - **Retry Logic**: Automatically retries failed idempotent requests
/// - **Deadline Management**: Tracks and enforces request deadlines
///
/// # Usage Patterns
///
/// The client supports multiple usage patterns:
///
/// 1. **Unary Calls**: Simple request-response operations
/// 2. **Server Streaming**: Request with streaming responses
/// 3. **Client Streaming**: Streaming requests with single response
...
/// - Channels can be shared across client instances
/// - Connection pool handles concurrent request routing
/// - Interceptors should be stateless for thread safety
c_wrapper!(CDMSCGrpcClient, DMSCGrpcClient);

/// Opaque C wrapper structure for DMSCGrpcChannel.
///
/// Low-level channel abstraction managing the underlying HTTP/2 connection pool, transport
/// security, and protocol negotiation.
///
/// # Channel Responsibilities
///
/// The gRPC channel manages:
///
/// - **Connection Pooling**: Maintains and reuses HTTP/2 connections
/// - **Load Balancing**: Distributes requests across connections
/// - **Health Tracking**: Monitors connection health and reconnect logic
/// - **Transport Security**: Handles TLS encryption and certificate validation
/// - **Protocol Negotiation**: Establishes HTTP/2 connections with proper settings
/// - **Keepalive**: Sends periodic pings to detect dead connections
///
/// # Connection Management
///
/// Channels implement sophisticated connection handling:
///
/// - **Pool Size**: Configurable number of connections to maintain
/// - **Lazy Connection**: Connections established on first request
/// - **Connection Health**: Active health checks detect failed connections
/// - **Automatic Reconnect**: Failed connections are recreated automatically
/// - **Load Distribution**: Requests distributed across connections
///
/// # Thread Safety
///
/// Channels are designed for thread-safe sharing:
///
/// - Multiple threads can use channel concurrently
/// - Connection pool handles thread-safe access
/// - Health checks run in background without blocking
/// - Concurrent requests use connection reservation
c_wrapper!(CDMSCGrpcChannel, DMSCGrpcChannel);

/// Creates a new DMSCGrpcChannel instance connecting to the specified server.
///
/// Initializes a gRPC channel with default connection pool settings. The channel will
/// establish connections as needed based on request volume.
///
/// # Parameters
///
...
/// # Returns
///
/// Pointer to newly allocated DMSCGrpcChannel on success, or NULL if:
/// - Memory allocation fails
/// - Invalid host or port parameters
///
/// # Default Configuration
///
/// The channel is configured with sensible defaults:
///
/// - Connection pool size: 4 connections
...
/// # Address Resolution
///
/// The host parameter is resolved using standard DNS resolution:
///
/// - Hostnames are resolved to IP addresses
/// - Multiple addresses result in connection attempts to each
/// - Failed addresses are retried with backoff
/// - IPv6 addresses are preferred if available
///
/// # Protocol Selection
///
/// The channel automatically selects the appropriate protocol:
///
/// - TLS is used when certificate configuration is provided
/// - HTTP/2 protocol is negotiated during connection establishment
/// - Protocol negotiation failures result in connection errors
c_constructor!(dmsc_grpc_channel_new, DMSCGrpcChannel, DMSCGrpcChannel, {
    let host = std::ffi::CStr::from_ptr(host);
    let host = host.to_str().map_err(|_| std::io::Error::new(std::io::ErrorKind::InvalidInput, "Invalid host encoding"))?;
    let port = port as u16;
    DMSCGrpcChannel::new(host, port)
});

/// Frees a previously allocated DMSCGrpcChannel instance.
///
/// Releases all connections in the pool and associated resources. In-flight requests
/// will be cancelled and return errors.
///
/// # Parameters
///
/// - `channel`: Pointer to DMSCGrpcChannel to free. NULL is safe and returns immediately.
///
/// # Preconditions
///
/// Before freeing a channel:
///
/// 1. Ensure all client operations using the channel are complete
/// 2. Close any active streams using the channel
/// 3. Release any client stubs created from the channel
///
/// # Behavior
///
/// The destructor performs the following:
///
/// - Closes all pooled connections gracefully
/// - Cancels any in-flight requests
/// - Releases connection pool resources
/// - Frees internal memory allocations
///
/// # Safety
///
/// Safe to call with NULL. Calling with already-freed pointer is undefined behavior.
c_destructor!(dmsc_grpc_channel_free, DMSCGrpcChannel);

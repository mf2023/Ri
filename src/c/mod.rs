...
/// Uses Arc<Mutex<>> to ensure thread-safe atomic operations (check-then-act).
static C_POINTER_REGISTRY: OnceLock<Arc<Mutex<HashSet<usize>>>> = OnceLock::new();

/// Get or initialize the global pointer registry
fn get_pointer_registry() -> &'static Arc<Mutex<HashSet<usize>>> {
    C_POINTER_REGISTRY.get_or_init(|| Arc::new(Mutex::new(HashSet::new())))
}

/// Register a pointer as valid
/// Returns the pointer for convenience
pub(crate) fn register_ptr(ptr: usize) -> usize {
    get_pointer_registry().lock().unwrap().insert(ptr);
    ptr
}
